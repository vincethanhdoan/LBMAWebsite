...
const { submitEnrollmentLeadWithTimeout, submitTrialBookingWithTimeout } =
  await import('./client');
...
describe('submitTrialBookingWithTimeout', () => {
  beforeEach(() => {
    fetchMock.mockReset();
  });
  ...
  it('sends the RPC parameter shape the DB function expects', async () => {
    fetchMock.mockResolvedValue(jsonResponse(receipt));
    await submitTrialBookingWithTimeout(trialInput, 5000);
    const [url, init] = fetchMock.mock.calls[0];
    expect(String(url)).toContain('/rest/v1/rpc/submit_trial_booking');
    expect(JSON.parse(init.body)).toEqual({
      p_parent_name: 'Test Parent',
      p_parent_email: 'parent@example.com',
      p_phone: '(209) 555-0123',
      p_children: [{ name: 'Kid', age: 6 }],
      p_bookings: [
        { program_type: 'youth', slot_id: 'slot-uuid', date: '2026-10-01' },
      ],
      p_request_id: 'request-uuid-123',
      p_language: 'en',
      p_message: null,
      p_source_page: 'contact',
    });
  ...
  it('sends the language and message when provided', async () => {
    fetchMock.mockResolvedValue(jsonResponse(receipt));
    await submitTrialBookingWithTimeout(
      { ...trialInput, language: 'es', message: 'Hola' },
      5000,
    );
  ...
  it('surfaces the already-booked code from the error body', async () => {
    fetchMock.mockResolvedValue(
      errorResponse(
        409,
        'Conflict',
        JSON.stringify({ message: 'already_booked', code: 'P0409' }),
      ),
    );
    const result = await submitTrialBookingWithTimeout(trialInput, 5000);
    expect(result.error).toEqual({ message: 'already_booked', code: 'P0409' });
  });

  it('surfaces the slot-taken code from the error body', async () => {
    fetchMock.mockResolvedValue(
      errorResponse(
        409,
        'Conflict',
        JSON.stringify({ message: 'slot_taken', code: '23P01' }),
      ),
    );
    const result = await submitTrialBookingWithTimeout(trialInput, 5000);
