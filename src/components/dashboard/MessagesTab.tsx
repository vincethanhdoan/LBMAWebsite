...
export function MessagesTab({ user }: MessagesTabProps) {
  ...
  const allowedDirectConversationIds = useMemo(
    () => convData?.allowedDirectIds ?? [],
    [convData],
  );
  ...
  const [directMessageTargets, setDirectMessageTargets] = useState<
    DirectMessageTarget[]
  >([]);
  const [selectedDirectTargetId, setSelectedDirectTargetId] = useState('');
  const [creatingDirectConversation, setCreatingDirectConversation] =
    useState(false);
  ...
  const [selectedParticipantId, setSelectedParticipantId] = useState<
    string | null
  >(null);
  const [participantFamilyData, setParticipantFamilyData] =
    useState<FamilyWithRelations | null>(null);
  const [loadingParticipantFamily, setLoadingParticipantFamily] =
    useState(false);
  const globalConvId =
    conversations.find((c) => c.type === 'group')?.id ?? null;
    ...
    const targets = allProfiles
      .filter(
        (profile) =>
          ...
          isDirectConversationAllowed(user.role, profile.role),
      )
      .map((profile) => ({
  ...
  const handleCreateDirectConversation = async () => {
    ...
    const target = directMessageTargets.find(
      (item) => item.userId === selectedDirectTargetId,
    );
    if (!target) return;
    ...
    try {
      ...
      toast.error(
        'Error creating direct conversation: ' + getErrorMessage(error),
      );
    } finally {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!canSendInSelectedConversation) {
      toast.error(
        'Attachments are only allowed in family-to-staff or staff-to-staff direct messages and the group chat.',
      );
      return;
    }
  ...
  return (
    <div
      className={`grid gap-4 h-[calc(100dvh-3.5rem-3rem)] ${showParticipants ? 'grid-cols-4' : 'md:grid-cols-3'}`}
    >
      {/* Conversations List */}
        ...
        <CardHeader className="shrink-0">
          ...
          <div className="space-y-2">
            <Select
              ...
              disabled={
                creatingDirectConversation || directMessageTargets.length === 0
              }
            >
                        ...
                        <div key={message.id}>
                          ...
                          >
                            ...
                            >
                              ...
                              {isLastInGroup && (
                                <p
                                  className={`text-xs text-muted-foreground px-3 mt-1 ${isOwnMessage ? 'text-right' : 'text-left'}`}
                                >
              ...
              {loadingParticipants ? (
                ...
                (() => {
                  const profile = allProfiles.find(
                    (p) => p.user_id === selectedParticipantId,
                  );
                  return (
                    <div className="p-4 space-y-4">
                      ...
                      {loadingParticipantFamily ? (
                        ...
                        participantFamilyData.students.length > 0 ? (
                  ...
                  {(['admin', 'family'] as const).map((role) => {
                    ...
                    return (
                      <div key={role}>
                        ...
                        {members.map((member) => {
                          const isClickable =
                            user.role === 'admin' && member.role === 'family';
                          return (
                            <button
                              key={member.userId}
                              type="button"
                              ...
                              onClick={() =>
                                isClickable &&
                                handleSelectParticipant(member.userId)
                              }
