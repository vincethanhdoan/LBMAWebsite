...
export function BlockedDates({
  ...
  const [removeBlockTarget, setRemoveBlockTarget] = useState<string | null>(
    null,
  );
  ...
  async function removeBlock(blockId: string) {
    const { error } = await supabase.rpc('remove_blocked_dates', {
      p_block_id: blockId,
    });
    if (error) {
  ...
  return (
    <section>
      ...
      <p className="text-[13px] text-muted-foreground mb-3">
        No bookings on these days. Existing appointments are not cancelled.
      </p>
      <Surface>
        {loading ? (
          ...
          blocks.map((b) => (
            ...
            >
              ...
              <span className="flex-1 min-w-0 text-[13px] text-muted-foreground truncate">
                {b.reason || 'No bookings'}
              </span>
              <Button
                size="icon"
                variant="ghost"
                className="h-8 w-8 text-destructive hover:text-destructive flex-shrink-0"
                onClick={() => setRemoveBlockTarget(b.block_id)}
              >
        ...
        <div className="mt-3 p-4 rounded border bg-muted/30 space-y-3">
          ...
          <div>
            ...
            <Input
              ...
              className="mt-1"
              placeholder="Holiday, closed, etc."
            />
          </div>
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => {
          if (!open) setRemoveBlockTarget(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                if (removeBlockTarget) removeBlock(removeBlockTarget);
              }}
            >
              Remove
            </AlertDialogAction>
