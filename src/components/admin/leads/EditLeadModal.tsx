...
function initialChildren(lead: EnrollmentLead): ChildRow[] {
  if (lead.children.length > 0) {
    return lead.children.map(c => ({ childId: c.child_id, name: c.name, age: String(c.age) }))
  }
  return [{ childId: null, name: lead.student_name ?? '', age: lead.student_age != null ? String(lead.student_age) : '' }]
}

export function EditLeadModal({ lead, onSuccess, onCancel }: EditLeadModalProps) {
  ...
  const [children, setChildren] = useState<ChildRow[]>(() => initialChildren(lead))
  ...
  async function handleSubmit() {
    ...
    try {
      await updateLead.mutateAsync({
        ...
        children: children.map(c => ({ childId: c.childId, name: c.name.trim(), age: Number(c.age) })),
      })
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input id="el-parent-name" value={parentName} onChange={e => setParentName(e.target.value)} onBlur={validate} className="mt-1" />
              {errors.parentName && <p className="text-xs text-destructive mt-1">{errors.parentName}</p>}
            </div>
            <div>
              <Label htmlFor="el-email">Email *</Label>
              <Input id="el-email" type="email" value={parentEmail} onChange={e => setParentEmail(e.target.value)} onBlur={validate} className="mt-1" />
              {errors.parentEmail && <p className="text-xs text-destructive mt-1">{errors.parentEmail}</p>}
            </div>
          </div>
          <div>
            <Label htmlFor="el-phone">Phone</Label>
            <Input id="el-phone" value={phone} onChange={e => setPhone(e.target.value)} className="mt-1" />
          </div>
          {/* Children */}
              ...
              return (
                <div key={i} className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <Input placeholder="Name" value={child.name} onChange={e => updateChildField(i,'name',e.target.value)} disabled={loading} required className="flex-1 h-9 text-sm" />
                    <Input type="number" min={4} max={17} placeholder="Age" value={child.age} onChange={e => updateChildField(i,'age',e.target.value)} disabled={loading} required className="w-16 h-9 text-sm" />
                    {children.length > 1 && (
          ...
          <p className="text-xs text-muted-foreground">
            Booking links are not changed automatically. Re-send invites if programs changed.
          </p>
        </div>
        <DialogFooter>
