...
export function useLeadActions({ onError }: { onError: (msg: string) => void }) {
  ...
  function markBusy(leadId: string) {
    setBusyLeadIds(s => { const next = new Set(s); next.add(leadId); return next; });
  }

  function clearBusy(leadId: string) {
    setBusyLeadIds(s => { const next = new Set(s); next.delete(leadId); return next; });
  }
        ...
        if (error || !data) {
          ...
          if (error instanceof FunctionsHttpError) {
            ...
            if (body?.code === 'slot_taken') {
              slotTakenProgram = programType ? PROGRAM_LABELS[programType] : 'this program';
              break;
            }
          }
