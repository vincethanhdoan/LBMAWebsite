-- send visit reminders per visit instead of once per lead ever
...
BEGIN
  ...
  WHERE el.deleted_at IS NULL
    ...
    AND EXISTS (
      SELECT 1 FROM enrollment_lead_program_bookings elpb
      WHERE elpb.lead_id = el.lead_id
        AND elpb.status IN (''scheduled'', ''confirmed'')
        AND elpb.appointment_date = ((now() AT TIME ZONE ''America/Los_Angeles'')::date + INTERVAL ''2 days'')::date
    )
    AND NOT EXISTS (
