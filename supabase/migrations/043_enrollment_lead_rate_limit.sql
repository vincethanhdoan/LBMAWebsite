-- Rate-limit public enrollment lead submissions.
-- Two guards inside submit_enrollment_lead (the only anon-callable insert path):
--   1. Per-email: one submission per 30 seconds for the same parent email.
--   2. Global: at most 20 leads created in any rolling hour.
-- Both raise SQLSTATE 'P0429' so the client can show a translated message.
