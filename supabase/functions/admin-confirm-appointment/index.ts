...
Deno.serve(async (req) => {
  ...
  if (readError) return new Response('Lookup failed', { status: 500, headers: cors })
  if (!bookings || bookings.length === 0) {
    const { data: lead } = await supabase
      ...
      .single()
    const legacyOk = lead
      && lead.appointment_date && lead.appointment_date >= todayPacific
      && lead.status === (revert ? 'appointment_confirmed' : 'appointment_scheduled')
    if (!legacyOk) return new Response('Nothing to update', { status: 422, headers: cors })
    const { error: legacyError } = await supabase
      .from('enrollment_leads')
      .update({ status: revert ? 'appointment_scheduled' : 'appointment_confirmed' })
      .eq('lead_id', leadId)
    if (legacyError) return new Response('Update failed', { status: 500, headers: cors })
  ...
  const { error: updateError } = await supabase
    ...
    .in('booking_id', bookings.map((b: { booking_id: string }) => b.booking_id))

  if (updateError) return new Response('Update failed', { status: 500, headers: cors })
