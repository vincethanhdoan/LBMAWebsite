...
type Lookup = { user_id: string; role: string; is_active: boolean; has_signed_in: boolean }
...
Deno.serve(async (req) => {
  ...
  const { data: lookupRows, error: lookupError } = await supabase.rpc('admin_lookup_email', {
  ...
  if (action === 'resend') {
    if (!existing || existing.role !== 'admin') return jsonError('No admin invite found for this email', 404, cors)
    if (existing.has_signed_in) return jsonError('This admin has already signed in', 409, cors)
    if (!existing.is_active) return jsonError('This admin is deactivated. Reactivate them instead.', 409, cors)
    ...
    if (otpError) {
      console.error('[invite-admin] resend otp error:', JSON.stringify(otpError))
      return jsonError('Could not send the email. Please try again shortly.', 500, cors)
    }
    await supabase
      .from('registered_emails')
      .update({ invited_at: new Date().toISOString(), invited_by_user_id: user.id })
  ...
  if (action === 'revoke') {
    ...
    if (existing.has_signed_in) {
      return jsonError('This admin has already signed in. Deactivate them instead.', 409, cors)
    }
    const { error: deleteError } = await supabase.auth.admin.deleteUser(existing.user_id)
    if (deleteError) {
      console.error('[invite-admin] revoke error:', JSON.stringify(deleteError))
      return jsonError('Could not revoke the invite. Please try again.', 500, cors)
    }
    return jsonOk(cors)
  }
  ...
  if (existing) {
    ...
    if (!existing.is_active) {
      return jsonError('This email belongs to a deactivated admin. Reactivate them instead.', 409, cors)
    }
    if (!existing.has_signed_in) {
      return jsonError("This admin was already invited and hasn't signed in yet. Use Resend invite instead.", 409, cors)
    }
  ...
  const { data: priorRow } = await supabase
    .from('registered_emails')
    .select('email, invited_by_user_id, invited_at, invitation_status, invited_as_role')
    .eq('email', normalizedEmail)
    .maybeSingle()
  ...
  if (name && typeof name === 'string' && name.trim()) inviteOptions.data = { display_name: name.trim() }
  ...
  if (inviteError) {
    ...
    if (priorRow) {
      await supabase.from('registered_emails').update(priorRow).eq('email', normalizedEmail)
    } else {
      await supabase.from('registered_emails').delete().eq('email', normalizedEmail)
    }
    return jsonError('Could not send the invite. Please try again shortly.', 500, cors)
  }
  ...
  if (inviteData?.user?.id) {
    ...
    if (confirmError) {
      ...
      return jsonError('Invite sent, but confirming the account failed. Ask them to use the portal login.', 500, cors)
    }
  }
